...
When photos are added via file upload, the code assigns an ID using `Date.now() + Math.random()`, which produces a non-integer. The delete handler then uses `parseInt` on the data-id, truncating the decimal portion. As a result, the stored `foto.id` (e.g. `169...0.123`) never equals the parsed integer (e.g. `169...0`), so uploaded photos cannot be deleted from the gallery even though camera-captured photos (integer IDs) still work. This only affects items added from the gallery path.
  }
